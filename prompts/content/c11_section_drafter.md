...
MARKUP RULES: write Markdown only. Never use raw HTML — no `<table>`, `<form>`, `<script>`, `<iframe>`, `<div>`, `<style>`. For comparison/spec tables use Markdown pipe tables (`| col | col |`). For lists use `-` / `*`. The renderer converts Markdown to clean HTML — raw HTML is escaped and shows as literal text on the page.
