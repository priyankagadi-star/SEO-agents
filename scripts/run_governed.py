"""Run pages GSC-governed, SEQUENTIALLY. GSC is auto-resolved from the account
(per-page export self-identifies via Filters.csv) — no --gsc needed. GOAL gate
enforced. Writes a governed scorecard. Usage: python scripts/run_governed.py [slug...]"""
import json, os, re, subprocess, sys
from pathlib import Path
sys.path.insert(0, str(Path.cwd()))

JOBS = ["features-chatgpt-visibility", "blog-track-brand-mentions",
        "blog-citation-rates", "blog-brand-monitoring"]
want = set(sys.argv[1:])
if want:
...
for slug in JOBS:
    ...
    print(f"\n=== {slug} (governed, GSC auto-resolved from account) ===", flush=True)
    before = run_ids()
